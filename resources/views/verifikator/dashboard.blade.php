...
@section('header', 'Verifikasi Pengajuan')
    ...
    @forelse($pengajuans as $pengajuan)
        <div style="border:1px solid #ccc; padding:15px; border-radius:5px; margin-bottom:20px;">
            ...
            <div style="margin-top:10px;">
                <a href="{{ route('verifikator.approve', $pengajuan->id) }}"
                ...
                <a href="{{ route('verifikator.reject', $pengajuan->id) }}"
                    onclick="return confirm('Yakin ingin menolak pengajuan ini?')"
                    style="padding:0.5rem 1rem; background-color:#dc3545; color:white; border-radius:5px; text-decoration:none; margin-left:10px;">Tolak</a>
            </div>
        </div>
    @empty
        <p>Tidak ada pengajuan yang perlu diverifikasi.</p>
    @endforelse
</div>
@endsection
